...
export function UploadHistoryList({ records }) {
    const [expandedId, setExpandedId] = useState(null);
    if (records.length === 0) {
        return (<EmptyState title="No datasets uploaded yet." description="Upload a CSV from Analyze Transaction to see batch results here."/>);
    }
    ...
    return (<div className="flex flex-col gap-3">
      {sorted.map((record) => {
            ...
            <button onClick={() => setExpandedId((id) => (id === record.id ? null : record.id))} className="flex w-full flex-col items-start gap-3 p-4 text-left sm:flex-row sm:items-center sm:justify-between sm:p-5">
            ...
            {isExpanded && (<div className="border-t border-ink-700 p-4 sm:p-6">
